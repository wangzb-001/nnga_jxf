...
class Net2(nn.Module):
    def __init__(self, dim, test_func: Problem_Abs):
        ...
        self.fc_out = nn.Linear(1 * (dim + 1), dim)
        self._init_weight()
...
class MutateGaModel:
    ...
    def init(cls, test_func, normalize=False, dim=5):
        ...
        cls.model = Net2(dim=dim, test_func=test_func)
    ...
    @classmethod
    def train_one(cls, input1, target, func, times=1):
        '''
        训练一次
    ...
    @classmethod
    def predict_one(cls, input1, func):
        i1f = func(input1)
        if isinstance(i1f, list) or isinstance(i1f, np.ndarray):
            if len(i1f.shape) == 1 and i1f.shape[0] == 1:
                i1f = i1f[0]
