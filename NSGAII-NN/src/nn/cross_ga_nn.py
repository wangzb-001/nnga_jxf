...
class Net(nn.Module):
    def __init__(self, dim):
        ...
        self.fc_input1 = nn.Linear(dim + object_nums, dim + object_nums)
        self.fc_input2 = nn.Linear(dim + object_nums, dim + object_nums)

        # self.fc_input_fv = nn.Linear(2, 2)
        self.activation = 'softmax'
        self.attention = SelfAttention(dim + object_nums)
        self.fc_out = nn.Linear(2 * (dim + object_nums), dim)
...
class CrossGaModel:
    ...
    @classmethod
    def init(cls, dim, normalize=False):
        cls.dim = dim
        cls.normalize = normalize
        cls.model = Net(dim=dim)
