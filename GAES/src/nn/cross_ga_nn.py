...
from src.problems.ES import ES
...
class Net(nn.Module):
    def __init__(self, dim, test_func: ES):
        '''
        :param dim:问题解 的维度
        '''
        self.dim = dim
    ...
    def forward(self, X):
        ...
        if self.activation == 'softmax':
            ...
            # input_fv = F.relu(self.fc_input_fv(input_fv))
        elif self.activation == 'sigmoid':
            x_1 = torch.sigmoid(self.fc_input1(input1))
            x_2 = torch.sigmoid(self.fc_input2(input2))
        else:
...
class CrossGaModel:
    ...
    def init(cls, test_func, normalize=False, dim=5):
        ...
        cls.optimizer, cls.criterion = optim.SGD(cls.model.parameters(), lr=0.1), torch.nn.BCELoss(reduction='none')
    ...
    def load_model(cls, path):
        cls.model.load_state_dict(torch.load(path), strict=True)

    @classmethod
    def train_one(cls, input1, input2, target, i1f, i2f, times=1):
        '''
        训练一次
        ...
        # i1f = func(input1)
        # i2f = func(input2)
        # target = [i for i, t in enumerate(target) if t == 1]
        ...
        for _ in range(times):
            ...
            for di in data:
                ...
                output = cls.model(di)
                _loss = cls.criterion(torch.sigmoid(output), target)
                loss = _loss.sum()
    ...
    @classmethod
    def predict_one(cls, input1, input2, p1_f, p2_f):
        i1f = p1_f
        i2f = p2_f
        if isinstance(input1, list):
            input1 = np.array(input1)
...
if __name__ == '__main__':
    CrossGaModel.init(32)
    input1 = np.random.randn(1, 32)
    input2 = np.random.randn(1, 32)
    target = np.random.randn(32)
    target = target.reshape(1, -1)
    print(target)
    # 100次之后预测效果
    for i in range(1000):
        loss = CrossGaModel.train_one(input1, input2, target)

    print(loss)
    p = CrossGaModel.predict_one(input1, input2)
    print(target)
    print(p)
    print(CrossGaModel.call_loss(target, p))
    #
    p = CrossGaModel.predict_one(input1 * 0.9, input2 * 1.1)
    print(p)
    print(CrossGaModel.call_loss(target, p))
